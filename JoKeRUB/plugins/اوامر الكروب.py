...
async def ban_user(chat_id, i, rights):
    try:
        await BlThon(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)        
@BlThon.ar_cmd(pattern="ارسل")
...
@BlThon.ar_cmd(pattern="حظر_الكل(?:\s|$)([\s\S]*)")
async def banall(event):
     ...
     is_admin = False
     try:
         partici_ = await BlThon(GetParticipantRequest(
           event.chat_id,
           event.sender_id
         ))
     ...
     usrnum = 0
     async for usr in BlThon.iter_participants(chat_id):
         ...
         if str(userb) == "None":
             userb = usr.id
             usrtxt = f"{msg} {userb}"
         await BlThon.send_message(chat_id, usrtxt)
     ...
     try:
         ...
         pass
@BlThon.ar_cmd(pattern="كتم_الكل(?:\s|$)([\s\S]*)")
async def muteall(event):
     ...
     try:
         ...
         pass
@BlThon.ar_cmd(pattern="طرد_الكل(?:\s|$)([\s\S]*)")
async def kickall(event):
     ...
     try:
         ...
         pass
@BlThon.ar_cmd(pattern="الغاء التفليش")
...
@BlThon.ar_cmd(pattern="مغادرة الكروبات")
async def Reda (event):
    ...
    try:
        ...
        for group in gr:
            if group not in dd:
                await BlThon.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
async def Hussein(event):
    if event.message.message.startswith("اطلع") and event.sender_id in DevJoker:
        ...
        if channel_username:
            try:
                entity = await BlThon.get_entity(channel_username)
                if isinstance(entity, Channel) and entity.creator or entity.admin_rights:
                    ...
                    await BlThon(LeaveChannelRequest(channel_username))
...
@BlThon.ar_cmd(pattern="مغادرة القنوات")
async def Hussein (event):
...
@BlThon.ar_cmd(pattern="تصفية الخاص")
async def hussein(event):
...
@BlThon.ar_cmd(pattern="تصفية البوتات")
async def Hussein(event):
...
is_Reham = False
No_group_Joker = "@BlackThonSupport1"
# يا يلفاشل هم الك نيه تاخذه وتنشره بسورسك 🤣
active_aljoker = []

@BlThon.ar_cmd(pattern=r"الذكاء تفعيل")
...
@BlThon.ar_cmd(pattern=r"الذكاء تعطيل")
...
async def Hussein(event):
    ...
    if sender_id != 705475246:
        if isinstance(event.message.entities, list) and any(isinstance(entity, MessageEntityCustomEmoji) for entity in event.message.entities):
            ...
            aljoker_entity = await BlThon.get_entity(sender.id)
...
@BlThon.ar_cmd(pattern="المميز تفعيل")
...
@BlThon.ar_cmd(pattern="المميز تعطيل")
async def disable_emoji_blocker(event):
    global Ya_Hussein
    Ya_Hussein = False
...
@BlThon.ar_cmd(pattern="منع_التفليش", require_admin=True)
async def Hussein_aljoker(event):
    addgvar("Mn3_Kick", True)
    await event.edit("**᯽︙ تم تفعيل منع التفليش للمجموعة بنجاح ✓**")

@BlThon.ar_cmd(pattern="سماح_التفليش", require_admin=True)
async def Hussein_aljoker(event):
    delgvar("Mn3_Kick")
    await event.edit("**᯽︙ تم تفعيل منع التفليش للمجموعة بنجاح ✓**")
message_counts = {}
...
async def handle_new_message(event):
    ...
    if message_counts[user_id]['count'] >= 3:
        try:
            await BlThon.edit_permissions(event.chat_id, user_id, send_messages=False)
            sender = await event.get_sender()
            aljoker_entity = await BlThon.get_entity(sender.id)
            aljoker_profile = f"[{aljoker_entity.first_name}](tg://user?id={aljoker_entity.id})"
...
async def handle_messages(event):
    if gvarstatus("5a9_dis"):
        sender_id = event.sender_id
        current_user_id = await BlThon.get_me()
        if event.is_private and sender_id != current_user_id.id:
            ...
            if sender_id not in aljoker_Menu:
                ...
                await BlThon.send_file(sender_id, aljoker_url, caption=f'**{aljoker_message}**\n**مدة الغياب: {aljoker_time}**')
                aljoker_Menu.add(sender_id)
@BlThon.ar_cmd(pattern="الخاص تعطيل")
...
@BlThon.ar_cmd(pattern="الخاص تفعيل")
async def joker5a9(event: Message):
    global afk_start_time
    delgvar("5a9_dis")
    afk_start_time = None
...
async def get_bot_entity():
    return await BlThon.get_entity('me')

@BlThon.ar_cmd(pattern="اسرع")
    ...
    if is_game_started and not is_word_sent and word.lower() in event.raw_text.lower():
        if event.chat_id:
            ...
            if bot_entity and event.sender_id != bot_entity.id:
                ...
                points_text = '\n'.join([f'{i+1}• {(await BlThon.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                await BlThon.send_message(event.chat_id, f'الف مبرووووك 🎉 الاعب ( {sender_first_name} ) فاز! \n اصبحت نقاطة: {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
joker = [
    "تلعب وخوش تلعب 👏🏻",
    "لك عاش يابطل استمر 💪🏻",
...
@BlThon.on(events.NewMessage(pattern=r'\طك (\d+)'))
...
@BlThon.on(events.NewMessage(pattern=r'\جيب (\d+)'))
    ...
    if chat_id in group_game_status and group_game_status[chat_id]['is_game_started2'] and event.sender_id == group_game_status[chat_id]['joker_player']:
        ...
        if 1 <= guess <= 6:
            if guess == correct_answer:
                ...
                points_text = '\n'.join([f'{i+1}• {(await BlThon.get_entity(participant_id)).first_name}: {participant_points}' for i, (participant_id, participant_points) in enumerate(sorted_points)])
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
                await BlThon.send_message(event.chat_id, f'الف مبروووك 🎉 الاعب ( {sender_first_name} ) وجد المحبس 💍!\n{format_board(game_board, numbers_board)}')
                game_board = [row[:] for row in original_game_board]
                await BlThon.send_message(event.chat_id, f'نقاط الاعب : {points[winner_id]}\nنقاط المشاركين:\n{points_text}')
            else:
                game_board = [["💍" if i == correct_answer - 1 else "🖐️" for i in range(6)]]
...
@BlThon.on(events.NewMessage(pattern=r'\انا'))
