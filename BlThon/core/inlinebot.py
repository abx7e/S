...
def main_menu():
    text = f"**- [سـورس بـلاكـثون](https://t.me/BlackThonSupport1)\
        \n المسـاعد\
        \n\
        \n◽ بـلاكـثون لـ {mention}**"
    buttons = [
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("BlThon"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" BlackThon UsᴇʀBoᴛ", "https://t.me/BlackThonSupport1"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./BlackThon", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس بـلاكـثون 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./BlackThon", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس بـلاكـثون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./BlackThon", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس بـلاكـثون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            _result = main_menu()
            result = builder.article(
                title="© BlackThon Help",
                description="Help menu for BlackThon",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/abx7e"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/BlackThonSupport1",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت بـلاكـثون 🧸♥](https://t.me/BlackThonSupport1)**\
            \n\
            \n❤ بـلاكـثون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/BlackThonSupport1)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            ...
            url="https://t.me/BlackThonSupport1",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
...
@BlThon.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@BlThon.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@BlThon.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@BlThon.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
