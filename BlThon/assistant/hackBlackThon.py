...
from BlackThon.utils import admin_cmd
import asyncio
...
'''
mm = '''
قم بلأنضمام الى قناة بـلاكـثون @abx7e
'''
...
keyboard = [
  ...
  [
    Button.url("سورس بـلاكـثون ", "https://t.me/abx7e")
    ]
]
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("هاك") and event.query.user_id == bot.uid:
            buttons = Button.url(" اضغط هنا عزيزي ", f"https://t.me/{BlackThon}?start=hack")
  ...
  if event.sender_id == bot.uid:
      async with bot.conversation(event.chat_id) as x:
        keyboard = [
          ...
          [
            Button.url("المطور", "https://t.me/abs7e")
            ]
        ]
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if len(i) > 1:
        ...
        await event.reply(i + "\n\nشكراً لأستخدامك سورس بـلاكـثون ❤️. \n/hack", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply(i + "\n\nشكرا لأستخدامك سورس بـلاكـثون", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      await event.reply("لقد تم حذف القناة/الكروب شكرا لأستخدامك بـلاكـثون.", buttons=keyboard)
...
async def users(event):
  async with bot.conversation(event.chat_id) as x:
      ...
      if i == True:
      	await event.reply("لقد تم انهاء جميع الجلسات شكراً لأستخدامك بـلاكـثون.", buttons=keyboard)
      else:
          await event.reply(f"حدث خطأ قم بتوجيه الرسالة للمطور @abs7e\n{i}")
...
async def start(event):
    keyboard = [
      ...
      [
        Button.url("القناة", "https://t.me/abs7e")
        ]
    ]
