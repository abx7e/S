...
from BlackThonUB import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"BlackThonUB/plugins/{shortname}.py")
        checkplugins(path)
        name = "BlackThonUB.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"BlackThonUB/plugins/{shortname}.py")
            name = f"BlackThonUB.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["BlackThonUB.plugins." + shortname] = mod
        LOGS.info("᯽︙تم بنجاح تحميل ملف ✓" + shortname)
...
def remove_plugin(shortname):
    ...
    try:
        name = f"BlackThonUB.plugins.{shortname}"
        for i in reversed(range(len(BlThon._event_builders))):
            ev, cb = BlThon._event_builders[i]
            if cb.__module__ == name:
                del BlThon._event_builders[i]
    except BaseException:
        raise ValueError
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"BlackThonUB/plugins/assistant/{shortname}.py")
        name = "BlackThonUB.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["BlackThonUB.plugins.assistant" + shortname] = mod
