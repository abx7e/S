...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @BlackThonSupport1", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@BlThon.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(BlackThonUB):
    dc = razan = BlackThonUB
    i = 0
    sender = await dc.get_sender()
    me = await BlackThonUB.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await BlackThonUB.get_chat()
    if BlackThonUB.is_private:
        user = BlackThonUB.chat
        rank = BlackThonUB.pattern_match.group(1)
    else:
        BlackThonUB.chat.title
    try:
        user, rank = await get_full_user(BlackThonUB)
    except:
        pass
    if me == user:
       BlackThon = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await BlackThonUB.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await BlackThonUB.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@BlThon.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(BlackThonUB):
    dc = razan = BlackThonUB
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       BlackThon = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
