...
@BlThon.ar_cmd(pattern="اضف صورة (الفحص|فحص) ?(.*)")
...
@BlThon.ar_cmd(pattern="اضف صورة (البنك|بنك) ?(.*)")
async def add_ping_aljoker(event):
    ...
    if reply and reply.media:
        ...
        await event.edit("**᯽︙ يرجى الرد على الصورة لتحديث الفار**")
@BlThon.ar_cmd(pattern="اضف صورة (الحماية|الحمايه|حماية|حمايه) ?(.*)")
...
@BlThon.ar_cmd(pattern="اضف صورة (الخاص|خاص) ?(.*)")
