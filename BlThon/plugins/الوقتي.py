...
from BlackThonUB import BOTLOG_CHATID
...
autopic_path = os.path.join(os.getcwd(), "BlackThonUB", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "BlackThonUB", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "BlackThonUB", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "BlackThonUB", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "BlackThonUB", "photo_pfp.png")
...
jep = Config.DEFAULT_PIC or "BlackThonUB/helpers/styles/PaybAck.ttf"
            ...
            if i > 0:
                await BlThon(
                    functions.photos.DeletePhotosRequest(
                        await BlThon.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            result = await BlThon(functions.photos.UploadProfilePhotoRequest(file=await BlThon.upload_file(autophoto_path)))
...
async def digitalgrouppicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await BlThon.upload_file(auto_group_photo_path)
        try:
            if i > 0:
                async for photo in BlThon.iter_profile_photos(int(dgp), limit=1) :
                    await BlThon(
            ...
            i += 1
            await BlThon(functions.channels.EditPhotoRequest(int(dgp), file))
            os.remove(auto_group_photo_path)
            await asyncio.sleep(60)
        except ChatAdminRequiredError:
            return await BlThon.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير صورة الكروب لتغيير صورة الكروب الوقتية •**")
...
async def group_loop():
    ...
    while AUTONAMESTAR:
        ...
        try:
            await BlThon(functions.channels.EditTitleRequest(
                channel=await BlThon.get_entity(int(ag)),
                title=name
            ))
        except ChatAdminRequiredError:
            await BlThon.tgbot.send_message(BOTLOG_CHATID, "**يجب ان يكون لديك صلاحية تغيير اسم الكروب لتفعيل وقتي الكروب•**")
        except ChannelInvalidError:
            return
        except FloodWaitError:
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await BlThon(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await BlThon(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@BlThon.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@BlThon.on(admin_cmd(pattern="كروب وقتي"))
async def _(event):
    ison = get_auto_g()
...
@BlThon.on(admin_cmd(pattern="كروب صورة وقتي"))
async def _(event):
...
@BlThon.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@BlThon.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
BlThon.loop.create_task(digitalpicloop())
BlThon.loop.create_task(digitalgrouppicloop())
BlThon.loop.create_task(autoname_loop())
BlThon.loop.create_task(autobio_loop())
BlThon.loop.create_task(group_loop())