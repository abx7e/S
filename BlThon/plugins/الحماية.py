...
# ترجمه وكتابة فريق بـلاكـثون
@BlThon.ar_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@BlThon.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@BlThon.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
# ترجمه وكتابة فريق بـلاكـثون
@BlThon.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@BlThon.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
# ترجمه وكتابة فريق بـلاكـثون
@BlThon.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "- - عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
...
@BlThon.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = " عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك "
        return await event.answer(text, cache_time=0, alert=True)
    ...
    try:
        ...
        PM_WARNS = {}  # ترجمه وكتابة فريق بـلاكـثون
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
@BlThon.ar_cmd(pattern="الحماية (تشغيل|تعطيل)$")  # ترجمه وكتابة فريق بـلاكـثون
...
async def pmpermit_on(event):
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None:  # ترجمه وكتابة فريق بـلاكـثون
...
@BlThon.ar_cmd(pattern="(س|سماح)(?:\s|$)([\s\S]*)")
async def approve_p_m(event):
    ...
    if not reason:
        reason = "لم يذكر"
    try:  # ترجمه وكتابة فريق بـلاكـثون
        PM_WARNS = sql.get_collection("pmwarns").json
    except AttributeError:
        PM_WARNS = {}
...
@BlThon.ar_cmd(pattern="(ر|رفض)(?:\s|$)([\s\S]*)")
...
@BlThon.ar_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@BlThon.ar_cmd(pattern="الخاص بلوك(?:\s|$)([\s\S]*)")
async def block_p_m(event):
    if gvarstatus("pmpermit") is None:
...
@BlThon.ar_cmd(pattern="انبلوك(?:\s|$)([\s\S]*)")
...
# ترجمه وكتابة فريق بـلاكـثون
@BlThon.ar_cmd(pattern="المسموح لهم$")
async def approve_p_m(event):
    if gvarstatus("pmpermit") is None:
        return await edit_delete(
            event,
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائمة الحماية بـلاكـثون.txt",
