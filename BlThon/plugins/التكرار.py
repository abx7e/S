...
async def spam_function(event, BlackThonUB, BlThon, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(BlThon[0])
    if len(BlThon) == 2:
        spam_message = str(BlThon[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await BlackThonUB.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and BlackThonUB.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            BlackThonUB = await event.client.send_file(
                event.chat_id, BlackThonUB, caption=BlackThonUB.text
            )
            await _catutils.unsavegif(event, BlackThonUB)
        ...
        if BOTLOG:
            ...
            BlackThonUB = await event.client.send_file(BOTLOG_CHATID, BlackThonUB)
            await _catutils.unsavegif(event, BlackThonUB)
        return
    elif event.reply_to_msg_id and BlackThonUB.text:
        spam_message = BlackThonUB.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@BlThon.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    BlackThonUB = await event.get_reply_message()
    BlThon = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(BlThon[0])
    except Exception:
    ...
    await spam_function(event, BlackThonUB, BlThon, sleeptimem, sleeptimet)

@BlThon.on(admin_cmd(pattern=f"{Mukrr}"))
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, BlThon, sleeptimem, sleeptimet, DelaySpam=True)


@BlThon.ar_cmd(pattern="تكرار الملصق$")
...
@BlThon.ar_cmd(pattern="ايقاف التكرار ?(.*)")
...
async def aljoker_nshr(BlThon, sleeptimet, chat, message, seconds):
    global yaAli
    yaAli = True
    while yaAli:
        if message.media:
            sent_message = await BlThon.send_file(chat, message.media, caption=message.text)
        else:
            sent_message = await BlThon.send_message(chat, message.text)
        await asyncio.sleep(sleeptimet)
@BlThon.ar_cmd(pattern="نشر")
async def Hussein(event):
    ...
    for chat_username in chat_usernames:
        try:
            chat = await BlThon.get_entity(chat_username)
            await aljoker_nshr(BlThon, seconds, chat.id, message, seconds)  # تمرير قيمة seconds هنا لكل مجموعة
...
async def aljoker_allnshr(BlThon, sleeptimet, message):
    global yaAli
    yaAli = True
    aljoker_chats = await BlThon.get_dialogs()
    while yaAli:
        for chat in aljoker_chats:
            if chat.is_group:
                if chat.title != "فريق الجوكر •Team ALjoker":
                    try:
                        if message.media:
                            await BlThon.send_file(chat.id, message.media, caption=message.text)
                        else:
                            await BlThon.send_message(chat.id, message.text)
                    except Exception as e:
                        print(f"Error in sending message to chat {chat.id}: {e}")
        await asyncio.sleep(sleeptimet)

@BlThon.ar_cmd(pattern="نشر_كروبات")
async def Hussein(event):
    await event.delete()
    ...
    global yaAli
    yaAli = True
    await aljoker_allnshr(BlThon, sleeptimet, message)
super_groups = ["super", "سوبر"]
async def aljoker_supernshr(BlThon, sleeptimet, message):
    global yaAli
    yaAli = True
    aljoker_chats = await BlThon.get_dialogs()
    while yaAli:
...
async def Hussein(event):
    ...
    global yaAli
    yaAli = True
    await aljoker_supernshr(BlThon, sleeptimet, message)
@BlThon.ar_cmd(pattern="ايقاف (النشر|نشر)")
