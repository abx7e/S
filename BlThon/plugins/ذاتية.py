...
@BlThon.on(admin_cmd(pattern="(جلب الصورة|جلب الصوره|ذاتيه|ذاتية)"))
...
- Dev: @abs7e
  """,
    ...
    await event.delete()
@BlThon.on(admin_cmd(pattern="(الذاتية تشغيل|ذاتية تشغيل)"))
...
@BlThon.on(admin_cmd(pattern="(الذاتية تعطيل|ذاتية تعطيل)"))
...
@BlThon.on(events.NewMessage(func=lambda e: e.is_private and joker_unread_media(e) and e.sender_id != bot.uid))
async def Reda(event):
    if gvarstatus("savepicforme"):
        caption = """**
