...
from BlackThonUB.utils import sudo_cmd
...
@BlThon.on(admin_cmd(pattern="تيك توك(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with bot.conversation(chat) as conv:
        ...
        await event.delete()
@BlThon.on(admin_cmd(pattern="ستوري(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with bot.conversation(chat) as conv:
        try:
            ...
            video = await conv.get_response()
            """ تم تحميل الستوري بنجاح من قبل @abx7e | @abs7e """
            await bot.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
        ...
        try:
            ...
            pass
        await bot.send_file(event.chat_id, video, caption="᯽︙ BY : @abx7e | @abs7e 🎀",parse_mode="html")
        await event.delete()
