...
@BlThon.on(admin_cmd(pattern="(نشر_تلقائي|النشر_التلقائي)"))
async def _(event):
    ...
    if jok.startswith("@"):
        ...
        BlackThonUB = jok.replace("https://t.me/", "@")
    elif str(jok).startswith("-100"):
        BlackThonUB = str(jok).replace("-100", "")
    else:
        try:
            BlackThonUB = int(jok)
...
async def _(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await BlThon.send_message(int(chat), event.message)
