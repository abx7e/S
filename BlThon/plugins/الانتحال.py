...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 CH : @abx7e"
)

@BlThon.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await BlThon.get_me()
...
jeps = ["BlackThonUB", "HuResupport"]
@BlThon.ar_cmd(pattern="انتحال_الدردشه")
async def reda(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await BlThon(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await BlThon(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await BlThon(functions.channels.EditTitleRequest(
                channel=await BlThon.get_entity(event.chat_id),
        ...
        try:
            await BlThon(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await BlThon.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await BlThon.upload_file(profile_pic)
        try:
            await BlThon(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @BlackThonSupport1")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
...
@BlThon.ar_cmd(pattern="اعادة_الدردشه")
async def reda_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in BlThon.iter_profile_photos(event.chat_id, limit=1) :
                    await BlThon(
