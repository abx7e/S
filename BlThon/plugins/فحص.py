...
@BlThon.ar_cmd(pattern="فحص(?:\s|$)([\s\S]*)")
async def amireallyalive(event):
    ...
    try:
        ...
        pass
    if HuRe_IMG:
        BlackThonUB = [x for x in HuRe_IMG.split()]
        PIC = random.choice(BlackThonUB)
        try:
...
**‌‎{EMOJI}BlackThon 𖠄 `{telever}`** ٫
**‌‎{EMOJI}uptime 𖠄 `{uptime}`** ٫
‌‎**{EMOJI}ping 𖠄 `{ping}`** ٫
‌‎**{EMOJI}setup 𝙳𝙰𝚃𝙴 𖠄 `{Tare5}`** ٫
**𖠄 BlackThon userbot 𖠄**"""
